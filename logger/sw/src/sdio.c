...
SD_Error SD_ReadBlock(uint32_t *readbuff, uint32_t ReadAddr, uint16_t BlockSize)
{
  ...
  SDIO_DataInitStructure.SDIO_DataBlockSize = (uint32_t)convert_from_bytes_to_power_of_two(BlockSize) << 4;
...
/**
  ...
  * @retval SD_Error: SD Card Error code.
  */
SD_Error SD_ReadMultiBlocks(uint32_t *readbuff, uint32_t ReadAddr, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
  SD_Error errorstatus = SD_OK;
  TransferError = SD_OK;
  TransferEnd = 0;
  ...
  SDIO_ITConfig(SDIO_IT_DATAEND, ENABLE);
  SDIO_DMACmd(ENABLE);
  SD_LowLevel_DMA_RxConfig(readbuff, (NumberOfBlocks * BlockSize));
...
/**
  ...
  * @retval SD_Error: SD Card Error code.
  */
SD_Error SD_WriteBlock(const uint32_t *writebuff, uint32_t WriteAddr, uint16_t BlockSize)
{
  SD_Error errorstatus = SD_OK;
...
SD_Error SD_WriteMultiBlocks(const uint32_t *writebuff, uint32_t WriteAddr, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
  ...
  SDIO_ITConfig(SDIO_IT_DATAEND, ENABLE);
  SDIO_DMACmd(ENABLE);    
  SD_LowLevel_DMA_TxConfig(writebuff, (NumberOfBlocks * BlockSize));
